...
| `https://query1.finance.yahoo.com/*`     | Used by the Stock and Watchlist widgets to fetch quote and chart data for stocks, ETFs, and crypto. Also used for symbol search.                                           |
| `https://query2.finance.yahoo.com/*`     | Reserved fallback for the Stock and Watchlist widgets. Yahoo's API rotates between `query1` and `query2`; both are declared so a future routing change doesn't break them. |
...
| **Stock**        | `query1.finance.yahoo.com`, `query2.finance.yahoo.com` | Quote + chart for stocks, ETFs, and crypto: only after you pick a symbol, then on the refresh interval you choose. Pauses outside US trading hours by default (crypto always refreshes). Symbol search: only when you submit a search. |
| **Watchlist**    | `query1.finance.yahoo.com`, `query2.finance.yahoo.com` | Same endpoints as the Stock widget. Fetches all configured symbols (stocks, ETFs, crypto) in parallel on the refresh interval you choose (default 2 min). No requests until you add at least one symbol.                               |

A Weather, Ping Monitor, Stock, or Watchlist widget that has been added but never configured produces **zero** network traffic. Ping Monitor uses `mode: "no-cors"` for its fetches, so it needs no host-specific manifest permissions — the trade-off is that responses are opaque (reachable / slow / unreachable, not HTTP status codes). The Stock widget's data comes from Yahoo Finance's undocumented public endpoints — the same endpoints the Python `yfinance` library uses. Free Yahoo data is typically delayed 15 minutes for US equities, and the widget surfaces this as a small "delayed" badge so you can always tell whether a price is fresh.
