...
# The extension now targets Firefox-first WebExtension APIs.
#
# SIMPLIFIED: No more manifest switching needed!
# The current manifest.json is configured for Firefox and compatible forks.
...
show_usage() {
    ...
    echo -e "The extension now uses a Firefox-focused Manifest V2 configuration."
    echo ""
...
show_status() {
    ...
    if [ -f "$ACTIVE_MANIFEST" ]; then
        ...
        echo -e "${GREEN}✅ Current manifest: Firefox WebExtension (Manifest V${MANIFEST_VERSION})${NC}"
        echo -e "   Version: $VERSION"
        echo -e "   File: extension/manifest.json"
        echo ""
        echo -e "${YELLOW}Browser Compatibility:${NC}"
        echo -e "   ✅ Firefox: Supported (Native Manifest V2)"
        echo -e "   ✅ Firefox-based browsers (Zen, LibreWolf, Waterfox)"
        echo ""
        echo -e "${BLUE}Features:${NC}"
        echo -e "   ✅ Unified API wrapper (browser-api.js)"
        echo -e "   ✅ webRequest-based header adjustment pipeline"
        echo -e "   ✅ No Chromium-specific declarativeNetRequest dependencies"
...
cleanup_legacy() {
    echo -e "${YELLOW}Cleaning up deprecated Chromium assets...${NC}"
    
    if [ -f "$CHROME_MANIFEST" ]; then
        rm "$CHROME_MANIFEST"
        echo -e "${GREEN}✅ Removed: $CHROME_MANIFEST (Chromium manifest)${NC}"
    fi
    ...
    if [ -f "$EXTENSION_DIR/frame-rules.json" ]; then
        ...
        echo -e "${GREEN}✅ Removed: frame-rules.json (Chromium DNR rules)${NC}"
    fi
