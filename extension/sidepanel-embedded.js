...
class EmbeddedSidepanel {
  ...
  renderWebsiteList() {
    ...
    const sortedWebsites = [...this.sidebarSettings.sidebarWebsites].sort((a, b) => a.position - b.position);
  ...
  setupMessageListeners() {
    window.addEventListener('message', (event) => {
      ...
      if (event.data.type === 'SIDEPANEL_URL_CHANGE') {
        ...
        if (newUrl) {
          console.log('URL change detected from content script:', this.currentWebsiteUrl, '→', newUrl);
