class StorageManager {
  ...
  async storeImage(imageData) {
    ...
    if (originalSize > 1024 * 1024) { // If larger than 1MB, try compression
      try {
        processedImageData = await this.compressImage(imageData);
        console.log(`Compressed image from ${Math.round(originalSize/1024)}KB to ${Math.round(this.estimateDataUrlSize(processedImageData)/1024)}KB`);
      } catch (error) {
    ...
    if (currentSize + imageSize > this.maxTotalSize) {
      throw new Error(`Storage limit exceeded. Current: ${Math.round(currentSize/1024)}KB, Adding: ${Math.round(imageSize/1024)}KB, Limit: ${Math.round(this.maxTotalSize/1024)}KB`);
    }
  ...
  async cleanupOrphanedImages(activeImageRefs) {
    ...
    const activeIds = new Set(
      activeImageRefs
        .filter(ref => ref && ref.type === 'indexeddb')
        .map(ref => ref.id)
    );
